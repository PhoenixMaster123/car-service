...
**[Try the demo](https://phoenixmaster123.github.io/car-service/app/)**
&middot;
[Documentation](https://phoenixmaster123.github.io/car-service/)
&middot;
[Walkthrough video](https://phoenixmaster123.github.io/car-service/video/car-service-demo.mp4)

![Sign in, book a service, pay, download the invoice, and the admin area](docs/demo.gif)

The demo is the application's own Thymeleaf markup and stylesheets running against a seeded
catalogue in the browser, so booking, paying, invoicing and the admin screens all work with no
backend behind them. The service catalogue is the same one `DataInitializer` writes on first boot.
State lives in `sessionStorage`, and reloading the page starts over on the sign-in screen.

Sign in as `alex@demo.dev` / `demo1234` for the customer side, or `admin@demo.dev` / `admin1234`
for the admin side — that one asks for a two-factor code, and any six digits will do. Both are
listed on the sign-in screen and fill the form when clicked.

What the demo cannot do, because it is static: Google and GitHub OAuth2 sign-in, Stripe payment
(the checkout runs the app's own dev-mode simulated payment instead), PDF invoice export
(openhtmltopdf runs server-side), and the Gemini-backed chatbot, which answers from the seeded
catalogue instead.
...
`.github/workflows/pages.yml` publishes <https://phoenixmaster123.github.io/car-service/> on every
push to `main`:

| Path | What |
| --- | --- |
| `/` | Landing page, from `docs/index.html` |
| `/app/` | The static demo, from `docs/app/` |
| `/javadoc/` | Generated by `mvn javadoc:javadoc` |
| `/checkstyle/` | Generated by `mvn checkstyle:checkstyle` |
| `/video/` | The walkthrough recording |

The demo's stylesheets under `docs/app/assets/` are copies of `src/main/resources/static`. When a
page's real stylesheet changes, copy it across and bump the `?v=` on the `<link>` and `<script>`
tags so browsers do not serve a stale mix.
